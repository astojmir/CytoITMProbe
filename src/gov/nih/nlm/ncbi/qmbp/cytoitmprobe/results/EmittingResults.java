...
import gov.nih.nlm.ncbi.qmbp.cytoitmprobe.selectors.ParticipationSelector;
import java.util.ArrayList;

public class EmittingResults extends ItmProbeResults {

    public EmittingResults(CyNetwork parentNetwork, String queryPrefix) {
        super(parentNetwork, queryPrefix);
        String [] s = {"Participation Ratio", "Cutoff Value", "Maximum Nodes"};
        selectionCriteria = s;
        defaultSelectionCriterion = 0;
        defaultNodeSelector = new ParticipationSelector(defaultMaxNodes);
    }
    ...
    protected ArrayList<String> getColFullNames() {
        ...
        for (String attr : validBoundary) {
            String name = String.format("Visits from %s", attr);
